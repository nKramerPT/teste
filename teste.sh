...
setup () {
  ...
  if [ "$yn" == "y" ]; then
    ...
    if [ $platform == "psn" ]; then
      ngrep -l -q -W byline -d $INTERFACE "psn-4" udp | grep --line-buffered -o -P 'psn-4[A-F0-9]{15}' | tee -a /tmp/data.txt &
...
if [ "$action" == "setup" ]; then
  ...
  sleep 1
  if [ $platform == "psn" ]; then
    ngrep -l -q -W byline -d $INTERFACE "psn-4" udp | grep --line-buffered -o -P 'psn-4[A-F0-9]{15}' | tee -a data.txt &
