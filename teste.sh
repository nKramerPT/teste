...
setup () {
  ...
  if [ "$yn" == "y" ]; then
    ...
    if [ $platform == "psn" ]; then
      ...
      ngrep -l -q -W byline -d $INTERFACE "psn-4" udp | grep --line-buffered -o -P 'psn-4[0]{20}' | tee -a /tmp/data.txt &
...
if [ "$action" == "setup" ]; then
  ...
  if [ $platform == "psn" ]; then
    ...
    ngrep -l -q -W byline -d $INTERFACE "psn-4" udp | grep --line-buffered -o -P 'psn-4[0]{15}' | tee -a data.txt &
  elif [ $platform == "xbox" ]; then
